...
func (a *AuthController) Login(c *gin.Context) {
	...
	// set httpOnly cookies for traditional cookie-based sessions
	c.SetCookie("access_token", access, 600, "/", "", false, true)
	c.SetCookie("refresh_token", refresh, 60*60*24*30, "/", "", false, true)

	// also return tokens in body so SPA can use Authorization header if needed
	response.OK(c, gin.H{"user": user, "access": access, "refresh": refresh})
}
